...
- **`.rubocop_todo.yml` is a record of current debt, not a budget.** It was generated by
  `--auto-gen-config`, so its `Metrics` maxima are wherever the code happened to sit. A task whose
  own additions push a file past one of them may raise that specific maximum, and must report which
  cop and by how much. Prefer extracting a well-named private method; do not contort naming or
  compress unrelated code to fit a snapshot. Gratuitous growth is still a review finding.
  (Amended mid-execution: the original "never edit it" wording forced an untested refactor in Task 4
  and single-letter locals in Task 6.)
